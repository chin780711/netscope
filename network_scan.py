...
def build_main_table(devices, full_scan=False):
    ...
    for i, device in enumerate(devices, start=1):
        if full_scan:
            ...
            table.add_row(str(i), ip, hostname, mac, vendor, device_type, open_ports)
        else:
            ip, mac, vendor, device_type = device
            table.add_row(str(i), ip, mac, vendor, device_type)
...
def main():
    ...
    for sent, received in result:
        ...
        if full_scan:
            ...
            devices.append(
                (ip, hostname, mac, vendor, device_type, open_ports)
            )  # 完整掃描固定 6 個值
